...
// BenchmarkDBValidate benchmarks validation operations
func BenchmarkDBValidate(b *testing.B) {
	// Create test database
	tmpDir := b.TempDir()
	...
	// Create multiple groups and targets
	for i := 0; i < 10; i++ {
		group := &db.Group{
			ConfigID:   cfg.ID,
			ExternalID: "bench-group-" + string(rune(i)),
			Name:       "Bench Group " + string(rune(i)),
		}
		...
		for j := 0; j < 5; j++ {
			target := &db.Target{
				GroupID: group.ID,
				Repo:    "mrz1836/bench-repo-" + string(rune(i*10+j)),
				Branch:  "main",
			}
