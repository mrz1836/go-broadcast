...
<body>
    ...
    <script>
        ...
        // Build status refresh functionality
        let buildStatusRefreshInterval;
        
        function startBuildStatusRefresh() {
            // Only start auto-refresh if we see an in-progress build
            const statusIndicator = document.querySelector('.status-indicator.build-running');
            if (!statusIndicator) return;
            
            // Start with an immediate refresh
            refreshBuildStatus().catch(error => {
                console.debug('Initial build status refresh failed:', error);
            });
            
            buildStatusRefreshInterval = setInterval(async () => {
                try {
                    await refreshBuildStatus();
                } catch (error) {
                    console.warn('Failed to refresh build status:', error);
                }
            }, 15000); // Refresh every 15 seconds
        }
        
        function stopBuildStatusRefresh() {
            if (buildStatusRefreshInterval) {
                clearInterval(buildStatusRefreshInterval);
                buildStatusRefreshInterval = null;
            }
        }
        ...
        async function refreshBuildStatus() {
            try {
                ...
                if (!response.ok) {
                    console.debug('Build status file not found, disabling auto-refresh');
                    stopBuildStatusRefresh();
                    return;
                }
                
                const data = await response.json();
                if (data && data.timestamp) {
                    // Check if the data is recent (within last 10 minutes)
                    const dataAge = Date.now() - new Date(data.timestamp).getTime();
                    if (dataAge > 10 * 60 * 1000) {
                        console.debug('Build status data is stale');
                        stopBuildStatusRefresh();
                        return;
                    }
                }
                ...
                console.debug('Build status refresh failed:', error);
                stopBuildStatusRefresh();
            }
        }
        
        function updateBuildStatusUI(buildStatus) {
            const statusIndicator = document.querySelector('.status-indicator');
            if (!statusIndicator) return;
            
            // Determine status details based on state and conclusion
            let iconHtml = '';
            let statusText = '';
            let className = 'status-indicator';
            let title = '';
            let workflowText = buildStatus.workflow_name || '';
            
            if (buildStatus.state === 'in_progress') {
                iconHtml = '<span class="status-icon spinning">🔄</span>';
                statusText = 'Build Running';
                className += ' build-running';
                title = 'View running build on GitHub Actions';
                if (buildStatus.run_number) {
                    workflowText += ' #' + buildStatus.run_number;
                }
            } else if (buildStatus.state === 'queued') {
                iconHtml = '<span class="status-icon">⏳</span>';
                statusText = 'Build Queued';
                title = 'View queued build on GitHub Actions';
            } else if (buildStatus.state === 'completed') {
                if (buildStatus.conclusion === 'success') {
                    iconHtml = '<span class="status-icon">✅</span>';
                    statusText = 'Build Passed';
                    className += ' build-success';
                    title = 'View successful build on GitHub Actions';
                } else if (buildStatus.conclusion === 'failure') {
                    iconHtml = '<span class="status-icon">❌</span>';
                    statusText = 'Build Failed';
                    className += ' build-failed';
                    title = 'View failed build on GitHub Actions';
                } else if (buildStatus.conclusion === 'cancelled') {
                    iconHtml = '<span class="status-icon">⚪</span>';
                    statusText = 'Build Canceled';
                    title = 'View canceled build on GitHub Actions';
                } else if (buildStatus.conclusion === 'stale') {
                    iconHtml = '<span class="status-icon">⚠️</span>';
                    statusText = 'Build Stale';
                    title = 'Build appears to be stuck';
                    workflowText += ' • May be stuck';
                } else {
                    iconHtml = '<span class="status-icon">🟡</span>';
                    statusText = 'Build ' + buildStatus.conclusion;
                    title = 'View build on GitHub Actions';
                }
                
                // Add duration if available
                if (buildStatus.duration) {
                    workflowText += ' • ' + buildStatus.duration;
                }
                
                // Stop refreshing once build is complete
                stopBuildStatusRefresh();
            } else {
                // Unknown state
                iconHtml = '<span class="status-icon">🔨</span>';
                statusText = buildStatus.state || 'Unknown';
                title = 'View build on GitHub Actions';
            }
            
            // Create the new status indicator content
            if (buildStatus.run_url) {
                // Create as a link
                const newIndicator = document.createElement('a');
                newIndicator.href = buildStatus.run_url;
                newIndicator.target = '_blank';
                newIndicator.className = className;
                newIndicator.title = title;
                newIndicator.innerHTML = iconHtml + 
                    '<div class="status-details">' +
                    '<span class="status-text">' + statusText + '</span>' +
                    '<span class="status-workflow">' + workflowText + '</span>' +
                    '</div>';
                
                // Replace the existing indicator
                statusIndicator.parentNode.replaceChild(newIndicator, statusIndicator);
            } else {
