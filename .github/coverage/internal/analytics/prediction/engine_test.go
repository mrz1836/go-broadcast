...
func TestPredictCoverage(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !tt.expectError {
				if len(result.Predictions) != tt.options.HorizonDays {
					t.Errorf("Expected %d predictions, got %d", tt.options.HorizonDays, len(result.Predictions))
				}
				...
				if tt.expectUpward && result.TrendDirection != "upward" {
					t.Errorf("Expected upward trend, got %s", result.TrendDirection)
				}
...
func TestTrainLinearModel(t *testing.T) {
	engine := NewPredictionEngine(&config.Config{})
...
func generateTestCommitSHA(index int) string {
	return fmt.Sprintf("commit%03d%s", index, "abcdef123456")[:40]
}