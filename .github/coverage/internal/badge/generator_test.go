...
func TestNew(t *testing.T) {
	...
	assert.Equal(t, 90.0, generator.config.ThresholdConfig.Excellent)
	assert.Equal(t, 80.0, generator.config.ThresholdConfig.Good)
	assert.Equal(t, 70.0, generator.config.ThresholdConfig.Acceptable)
	assert.Equal(t, 60.0, generator.config.ThresholdConfig.Low)
}
...
func TestRenderFlatBadge(t *testing.T) {
	...
	svg := generator.renderFlatBadge(data, 100, 20, 50, 40, 0)
	svgStr := string(svg)
...
func TestRenderWithLogo(t *testing.T) {
	...
	svg := generator.renderFlatBadge(data, 120, 20, 50, 40, 16)
	svgStr := string(svg)
