...
Generated by [GoFortress Coverage System](https://github.com/mrz1836/go-broadcast).
`
	...
	// Commit and push
	if err := d.commitAndPush(ctx, "🚀 Initialize GitHub Pages structure"); err != nil {
...
func (d *GitHubPagesDeployer) Deploy(ctx context.Context, opts DeployOptions) error {
	...
	if message == "" {
		if opts.PRNumber != "" {
			message = fmt.Sprintf("📊 Update coverage for PR #%s", opts.PRNumber)
		} else {
			message = fmt.Sprintf("📊 Update coverage for %s branch", opts.Branch)
		}
	}
...
func (d *GitHubPagesDeployer) Clean(ctx context.Context, opts CleanOptions) error {
	...
	if len(removed) > 0 && !opts.DryRun {
		message := fmt.Sprintf("🧹 Clean up %d expired PR directories", len(removed))
		if err := d.commitAndPush(ctx, message); err != nil {
...
<head>
    ...
    <title>🏰 GoFortress Coverage Dashboard</title>
    <style>
        :root {
            --bg-primary: #0d1117;
            --bg-secondary: #161b22;
    ...
    <div class="container">
        ...
        <main>
            <div class="status-message">
                <h2>🚀 Welcome to GoFortress Coverage!</h2>
        ...
        <footer class="footer">
            <p>📊 Generated by <strong>GoFortress Coverage System</strong></p>
